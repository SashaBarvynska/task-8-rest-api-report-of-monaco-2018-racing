...
from src.serializers import data_json, data_xml
...
def show_report() -> wrappers.Response:
    ...
    if request.args.get('format') == "json":
        ...
        return make_xml_response(data_xml(request.args.get('order')), HTTPStatus.OK)


@app.route('/report/drivers', methods=['GET'])
@swag_from('swagger/drivers.yml')
def show_drivers() -> wrappers.Response:
    dict_of_lists = data_json(request.args.get('order'))
    if request.args.get('format') == "json":
...
def get_info(driver_code: str) -> wrappers.Response:
    instance = DriverAdaptor()
    driver = instance.get_driver("abbreviation", driver_code.upper())
    if driver is None:
